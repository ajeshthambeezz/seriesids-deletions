        ...
        if [ $(echo "$seriesResponse" | jq '.response | length') -eq 0 ]; then
            ...
            if  [ "$season_number" == "All" ]; then
                ...
                for season_id in $SereisSeasonIDs; do
                    ...
                    delete_episodes "$season_id" "$LOG_FILE" "$JSESSIONID" ""
                    delete_season "$season_id" "$LOG_FILE" "$JSESSIONID" ""
                done
                ...
                delete_episodes "$selected_season_id" "$LOG_FILE" "$JSESSIONID" ""
                # Delete the selected season ID after deleting episodes
                delete_season "$selected_season_id" "$LOG_FILE" "$JSESSIONID" ""
            fi
        fi
    else
        ...
        for episodeID in $episodeIDs; do
            ...
            if jq -e . >/dev/null 2>&1 <<<"$delete_episode_response"; then
                ...
                if [ "$status_code" == "0" ]; then
                    ...
                    echo "{\"episodeID\": \"$episodeID\", \"statusMessage\": \"$statusMessage\"}" >> ""
                fi
            else
...
delete_season() {
    ...
    if jq -e . >/dev/null 2>&1 <<<"$delete_season_response"; then
        ...
        if [ "$status_code" == "0" ]; then
            ...
            # Record failed response
            echo "{\"seasonID\": \"$SEASON_ID\", \"statusMessage\": \"$statusMessage\"}" >> ""
        fi
    else
...
delete_all_seriesIds() {
    ...
    if jq -e . >/dev/null 2>&1 <<<"$seasonResponse"; then
        ...
        if [[ $(echo "$seasonResponse" | jq '.response | length') -eq 0 ]]; then
            ...
            for season_id in $(echo "$seasonResponse" | jq -r '.response[].id'); do
                ...
                # Call the listEpisodes_imageDeletion function for each season ID
                listEpisodes_imageDeletion "$season_id" "$LOG_FILE" "$JSESSIONID"
    ...
    if jq -e . >/dev/null 2>&1 <<<"$deleteSeries"; then
        ...
        # Check the status code to determine success or failure
        if [ "$status_code" == "1" ]; then
            echo "Failed to delete series $SERIES_ID. Error message: $error_message errorCode: $errorCode  SeriesID might be Deleted"  | tee -a "$LOG_FILE"
            echo "" | tee -a "$LOG_FILE"
...
if [ -f "$1" ]; then
    ...
    while IFS= read -r SERIES_ID; do
        ...
        # Call delete_series function for each series ID in the series_ids.txt file
